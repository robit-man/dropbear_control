...
`13cf5ecaa39b8b89c794fe905dcea0490cfa7726` and the corrected USD from
`robit-man/dropbear-locomotion` at revision
`a397be863fed2d328c2e8f62c3db2f1e23575eb1`.
...
- **Live Robot** — the tracked `dropbear.usd`, adapted into a lightweight
  browser cache while retaining source manifests for 93 rigid bodies, 93
  authored masses/inertias, 93 collision groups, 117 physical joints, 28
...
- **Controller Health** — a live two-ESP32 functional schematic showing the
  passive USB readers, strict degree parser, five ADC channels per side,
  calibration gates, corrected USD state path, unavailable telemetry, and the
  locked serial/CAN transmit boundary.
- **Firmware** — two-controller serial console, exact command grammar,
  ...
  fault controls.
- **Policy playback** — the live robot view retains playback plumbing for
  artifacts exported by `dropbear-locomotion`; browser training labs are not
  exposed in the control workspace.
...
viewport. The corrected source authors `RL_Revolute81` as Z, so the browser
consumes the USD axis directly with no local kinematic override.
...
<https://github.com/robit-man/dropbear-locomotion> and is licensed
